  ...
  return (
    <div className="play-roll-card">
      ...
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="play-h-display text-lg">{prompt.label}</h3>
        {prompt.vsLabel || prompt.dc != null ? (
          <Pill variant="tint">{prompt.vsLabel ?? `DC ${prompt.dc}`}</Pill>
        ) : null}
      </div>
      <p className="play-mono text-[0.6875rem] leading-snug text-[var(--ink-3)]">
