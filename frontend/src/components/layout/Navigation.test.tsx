...
describe('Navigation', () => {
  ...
  it('calls onItemClick when item is clicked', async () => {
    ...
    render(<Navigation items={mockItems} onItemClick={onItemClick} />);
  ...
  it('marks active item with aria-current', () => {
    ...
    // Both mobile and desktop render, check mobile menu item
    const activeItems = screen.getAllByText('Characters');
    const mobileActiveItem = activeItems.find(item => 
      item.closest('.md\\:hidden') !== null || 
      item.closest('[id="mobile-menu"]') !== null
    ) || activeItems[0];
    expect(mobileActiveItem).toHaveAttribute('aria-current', 'page');
  });
  ...
  it('has proper ARIA labels', () => {
    render(<Navigation items={mockItems} mobileMenuLabel="Custom nav" />);
  ...
  it('focuses first menu item when mobile menu opens', async () => {
    ...
    const homeItems = screen.getAllByText('Home');
    const mobileItem = homeItems.find(item => 
      item.closest('[id="mobile-menu"]') !== null
    );
  ...
  it('renders nothing when items is undefined', () => {
    const { container } = render(<Navigation items={undefined as any} />);
    expect(container.firstChild).toBeNull();
  });
});
